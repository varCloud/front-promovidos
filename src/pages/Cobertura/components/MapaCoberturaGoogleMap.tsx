...
const _LoadingContainer = () => <div key='fance loading'>Fancy loading container!</div>;
			...
			<Map
					...
					if (p.latitud && p.longitud) {
						return (
							<MarkerWithInfoWindow
								colorMarker={colorsMarker[p.Usuario.idRol]}
								latLng={{ lat: Number(p.latitud), lng: Number(p.longitud) }}
								info={`${p.Usuario.nombres} ${p.Usuario.apellidos ?? ''}`}
							/>
						);
					}
					return null;
				...
				{promovidos.map((p) => {
					if (p.latitud && p.longitud) {
						return (
							<MarkerWithInfoWindow
								colorMarker={colorsMarker[p.idRol]}
								latLng={{ lat: Number(p.latitud), lng: Number(p.longitud) }}
								info={`${p.nombres} ${p.apellidos ?? ''}`}
							/>
						);
					}
					return null;
				...
				{casillas.map((p) => {
					if (p.latitud && p.longitud) {
						return (
							<MarkerWithInfoWindow
								ubicacion={`${p.tipoCasilla} ${p.ubicacion}`}
								backgroundColor={
									BackgroundColorCasilla[p.distritoLocal.split('.')[0]]
								}
								colorMarker={colorsMarker.casillas}
								latLng={{ lat: Number(p.latitud), lng: Number(p.longitud) }}
								info={`${p.domicilio}`}
							/>
						);
